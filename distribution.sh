...
####  perl calculate_midpoint.pl ${samp}.peak.bed ${samp}.midpoint.txt
...
##  convert Ensembl ID into NCBI ID
  perl ENST_to_refseq.pl ENST_to_refseq.txt ${samp}_ENST.txt ${samp}_refseq.txt
...
##  calculate relative coordination on virtual transcript and count sites within each bin for drawing of distribution profile
  perl calculate_distribution.pl ${samp}_refseq_annotated.xls ${samp}_distribution.xls
done